...
class Region(str, Enum):
    ...
    Each region has different tile sizes and resolutions:
                    ...
                    NRW                 RLP                 BB                  BW                  BY                 TH                 CZ
    Tile size       1km x 1km           2km x 2km           1km x 1km           2km x 2km           1km x 1km          1km x 1km        2km x 2km
    Orthophoto      0.1 m/px            0.2 m/px            (not available)     0.2 m/px            0.2 m/px           0.2 m/px         INSPIRE OI
    DSM spacing     0.5 m               0.2 m               0.2 m (raster)      1.0 m (raster)      0.2 m (raster)     1.0 m (raster)   0.5 m (raster)

    NRW/RLP/BW/BY/TH use EPSG:25832 (UTM Zone 32N). BB and CZ use zone 33 products.
    """
...
CZ_GRID_SIZE = 2000  # 2km INSPIRE tiles
...
LAZ_SPACING_BY_REGION = {
    ...
    Region.CZ: 0.5,  # DMPOK TIFF is 0.5m raster; DMP1G LAZ is only fallback
}

LAZ_SAMPLE_SIZE = 10000  # Points to sample for verification

# Product-level native rasterization targets for irregular LAZ fallback sources.
# These are intentionally not region-wide because a region can mix regular-grid DSM
# products (e.g. DMPOK TIFF) with irregular fallback LAZ products (e.g. DMP1G).
IRREGULAR_LAZ_NATIVE_RESOLUTION_BY_PRODUCT = {
    "cz_dmp1g": 1.0,
}

IRREGULAR_LAZ_DEFAULT_SRS_BY_PRODUCT = {
    "cz_dmp1g": "EPSG:3045",
}
...
CZ_OI_PATTERN = re.compile(r"oi_(\d{3})_(\d{4})_(\d{4})\.jp2$", re.IGNORECASE)
CZ_DMPOK_PATTERN = re.compile(r"dmpok_(\d{3})_(\d{4})_(\d{4})\.tif$", re.IGNORECASE)
CZ_DMP1G_PATTERN = re.compile(r"dmp1g_(\d{3})_(\d{4})_(\d{4})\.(laz|tif)$", re.IGNORECASE)
